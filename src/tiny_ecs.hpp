...
public:
	...
	inline Component& insert(Entity e, Component c, bool check_for_duplicates = true)
	{
		...
		map_entity_component_id[e] = (unsigned int)components.size();
	...
	template <typename... Args> Component& emplace_with_duplicates(Entity e, Args&&... args)
	{
	...
	Component& get(Entity e)
	{
		...
		return components[map_entity_component_id[e]];
	}
	...
	void remove(Entity e) override
	{
		if (has(e)) {
			...
			int cID = map_entity_component_id[e];
			...
			map_entity_component_id[entities.back()] = cID;

			// Erase the old component and free its memory
			map_entity_component_id.erase(e);
			components.pop_back();
			entities.pop_back();
	...
	void clear() override
	{
		map_entity_component_id.clear();
		components.clear();
		entities.clear();
	}

	// Report the number of components of type 'Component'
	size_t size() override { return components.size(); }
