...
void RenderSystem::initialize_room_vertices(MapUtility::RoomType roomType)
{
	...
	*/
	...
	for (int i = 0; i < total_vertices; i += 6) {
		...
		tilemap_vertices[i + 0].position = { fraction * (col - MapUtility::room_size / 2), fraction * (MapUtility::room_size / 2 - row), 0.f };
		tilemap_vertices[i + 1].position
			= { fraction * (col - MapUtility::room_size / 2), fraction * (MapUtility::room_size / 2 - row - 1), 0.f };
		tilemap_vertices[i + 2].position
			= { fraction * (col - MapUtility::room_size / 2 + 1), fraction * (MapUtility::room_size / 2 - row), 0.f };
		tilemap_vertices[i + 3].position
			= { fraction * (col - MapUtility::room_size / 2 + 1), fraction * (MapUtility::room_size / 2 - row), 0.f };
		tilemap_vertices[i + 4].position
			= { fraction * (col - MapUtility::room_size / 2), fraction * (MapUtility::room_size / 2 - row - 1), 0.f };
		tilemap_vertices[i + 5].position
			= { fraction * (col - MapUtility::room_size / 2 + 1), fraction * (MapUtility::room_size / 2 - row - 1), 0.f };
		...
		// The Room somehow was read upside down, so using MapUtility::room_size - row - 1 to reverse that, should
		// investigate why later...
		tilemap_vertices[i + 0].tile_texture = (float)(room_layouts.at(roomType).at(MapUtility::room_size - row - 1).at(col));
		tilemap_vertices[i + 1].tile_texture = (float)(room_layouts.at(roomType).at(MapUtility::room_size - row - 1).at(col));
		tilemap_vertices[i + 2].tile_texture = (float)(room_layouts.at(roomType).at(MapUtility::room_size - row - 1).at(col));
		tilemap_vertices[i + 3].tile_texture = (float)(room_layouts.at(roomType).at(MapUtility::room_size - row - 1).at(col));
		tilemap_vertices[i + 4].tile_texture = (float)(room_layouts.at(roomType).at(MapUtility::room_size - row - 1).at(col));
		tilemap_vertices[i + 5].tile_texture = (float)(room_layouts.at(roomType).at(MapUtility::room_size - row - 1).at(col));
	}
