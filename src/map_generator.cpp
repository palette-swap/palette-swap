								 ...
								 bool is_debugging)
{
	...
	level_conf.map_layout.at(starting_node->position / map_size).at(starting_node->position % map_size) = static_cast<RoomID>(level_conf.room_layouts.size() - 1);
										   ...
										   bool will_generate_boss_room)
{
	...
	auto get_available_positions_from_current_room = [&]() {
		...
		if (curr_room->room_type == RoomType::Big) {
			...
			if (room_col - 1 >= 0 && room_row + 1 < map_size && visited_rooms.find(room_position + map_size - 1) == visited_rooms.end()) {
			...
			if (room_row - 1 >= 0 && room_col + 1 < map_size && visited_rooms.find(room_position - map_size + 1) == visited_rooms.end()) {
				positions.emplace_back(room_position - map_size + 1);
			}
			if (room_row + 2 < map_size && room_col + 1 < map_size && visited_rooms.find(room_position + map_size * 2 + 1) == visited_rooms.end()) {
			...
			if (room_row + 1 < map_size && room_col + 2 < map_size && visited_rooms.find(room_position + map_size + 2) == visited_rooms.end()) {
				positions.emplace_back(room_position + map_size + 2);
			}
		} else {
		...
		if (path_length == 2 && will_generate_boss_room) {
			...
			for (const auto & big_room_offset : big_room_vec) {
				...
				if ((room_row >= 0 && room_col >= 0 && room_row + 1 < room_size && room_col + 1 < room_size)
					...
					if (generate_path_from_node(next_room, path_length - 1, visited_rooms, random_eng, room_type, false)) {
						return true;
					}
					// backtrack
			...
			if (generate_path_from_node(next_room, path_length - 1, visited_rooms, random_eng, room_type, will_generate_boss_room)) {
				return true;
			}
			// backtrack
...
LevelConfiguration MapGenerator::generate_level(LevelGenConf level_gen_conf, bool is_debugging)
{
	...
	if (!generate_path_from_node(starting_room, level_gen_conf.level_path_length - 1, visited_rooms, random_eng, RoomType::Critical, level_gen_conf.room_difficulty > 1))
	{
	...
	while (curr_room->children.size()) {
		...
		for (int i = 0; i < num_side_rooms_to_generate; i ++) {
			...
			room_length = (room_length < 0 || level_gen_conf.side_room_percentage == 0) ? 0 : (room_length > 9.95) ? 10 : room_length; 
